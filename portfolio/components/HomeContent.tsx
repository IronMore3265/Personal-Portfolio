...
export default function HomeContent({ scrollTo }: { scrollTo?: string }) {
  const heroRef = useHeroAnimation();
  const revealRef = useRevealAnimations();

  // When landed on a clean section route (/about, /experience, …), jump to
  // that section after layout. Instant (not smooth) since it's an initial load.
  useEffect(() => {
    if (!scrollTo) return;
    const raf = requestAnimationFrame(() =>
      scrollToSection(scrollTo, "instant" as ScrollBehavior)
    );
    return () => cancelAnimationFrame(raf);
  }, [scrollTo]);

  // Smooth-scroll to Contact with a clean URL instead of remounting the page.
  ...
  return (
    <div ref={revealRef}>
      ...
      <section
        className="py-24 md:py-section-gap px-margin-mobile md:px-margin-desktop max-w-[1440px] mx-auto scroll-mt-24 md:scroll-mt-32"
        id="about"
      >
      ...
      {/* ===== EXPERIENCE SECTION ===== */}
      <section
        className="w-full py-24 md:py-section-gap px-margin-mobile md:px-margin-desktop relative overflow-hidden scroll-mt-24 md:scroll-mt-32"
        id="experience"
      >
      ...
      {/* ===== CONTACT SECTION ===== */}
      <section
        className="py-24 md:py-section-gap px-margin-mobile md:px-margin-desktop bg-surface-container-lowest border-t border-outline-variant text-center dark:bg-surface-container scroll-mt-24 md:scroll-mt-32"
        id="contact"
      >
